...
void moloch_config_load()
{
    ...
    config.fileExtractDir       = moloch_config_str(keyfile, "fileExtractDir", "./output");
    ...
    //config.cpuAffinityStart      = moloch_config_int(keyfile, "cpuAffinityStart", 0, 0, MOLOCH_MAX_PACKET_THREADS);
    config.cpuAffinityStart      = moloch_config_int(keyfile, "analysisCpuStartIndex", 0, 0, MOLOCH_MAX_PACKET_THREADS);
    config.maxLimitEnqueue	 = (config.maxPacketsInQueue * config.packetThreads) * 0.9;
    LOG("maxPacketsInQueue : %"PRIu32", maxLimitEnqueue : %"PRIu32, config.maxPacketsInQueue, config.maxLimitEnqueue);
#endif
