...
static char *extract_json[MOLOCH_MAX_PACKET_THREADS];
static int extract_json_len[MOLOCH_MAX_PACKET_THREADS];
void moloch_db_save_file_extract(MolochExtractFile_t *info)
{
#ifdef TESTMODE
    printf("Save files to database\n");
    printf("%s,%s,%"PRIu64",%lu,%lu,%lu,%lu,%u,%u",
            info->type,
            info->id,
            info->time,
            info->times,
            ...
            info->p2);
#endif
    if (!extract_json[info->thread]) {
        extract_json[info->thread] = moloch_http_get_buffer(MOLOCH_HTTP_BUFFER_SIZE);
    }
    struct tm tmp;
    gmtime_r((time_t *)&info->times, &tmp);
    char buff_head[1024];
    char buff_tail[1024];

    sprintf(buff_head, "{\"index\":{\"_index\":\"attach-%02d%02d%02d\","
                    "\"_type\":\"%s\",\"_id\":\"%s\"}}\n"
                    "{\"time\":%"PRIu64","
                    "\"times\":%lu,"
                    "\"timeu\":%lu,"
                    ...
                    info->type,info->id,
                    ((uint64_t)info->times)*1000 + ((uint64_t)info->timeu)/1000,
                    info->times, info->timeu,
                    info->a1, info->a2, info->p1, info->p2);
    if (strcmp(info->type, "http") == 0) {
        sprintf(buff_tail, ",\"method\":\""
                            "METHOD"
                            "\",\"host\":\""
                            "HOST"
                            "\",\"cont_type\":\""
    ...
    else if (strcmp(info->type, "ftp") == 0) {
        sprintf(buff_tail, ",\"filename\":\"%s"
                        "\",\"dataip\":%u"
                        ",\"dataport\":%u"
                        ",\"fpd\":%llu"
                        ",\"lpd\":%llu"
    ...
    else if (strcmp(info->type, "pop") == 0
            || strcmp(info->type, "smtp") == 0) {
        sprintf(buff_tail, ",\"eml_path\":\"%s"
                            "\",\"eml_err\":%d"
                            ",\"path\":\"%s"
                            "\",\"name\":\"%s"
                            "\"}\n"
    ...
    sprintf(extract_json[info->thread] + extract_json_len[info->thread], "%s%s", buff_head, buff_tail);
    extract_json_len[info->thread] += strlen(buff_head) + strlen(buff_tail);
#ifndef TESTMODE
    if (extract_json_len[info->thread] + 1024 >= MOLOCH_HTTP_BUFFER_SIZE) {
#endif
#ifdef TESTMODE
        printf("%s\n", extract_json[info->thread]);
#endif
        moloch_http_set(esServer, "/_bulk", 6, extract_json[info->thread], extract_json_len[info->thread], NULL, NULL);
        extract_json_len[info->thread] = 0;
        extract_json[info->thread] = NULL;
#ifndef TESTMODE
    }
#endif
}
...
/******************************************************************************/
static char *fingerprint_json[MOLOCH_MAX_PACKET_THREADS];
static int fingerprint_json_len[MOLOCH_MAX_PACKET_THREADS];
void moloch_db_save_fingerprint(MolochFpHost_t *host)
{
    struct tm tmp;
    char tmpbuff[256];
    int i;
    ...
    json[0] = '\0';
    size_t times = host->times;
    gmtime_r((time_t *)&times, &tmp);
    char *ip = addr_to_str(host->addr, host->ip_ver);

    if (!fingerprint_json[host->thread]) {
        fingerprint_json[host->thread] = moloch_http_get_buffer(MOLOCH_HTTP_BUFFER_SIZE);
    }
    sprintf(tmpbuff, "{\"index\":{\"_index\":\"fingerprint-%02d%02d%02d\","
                    "\"_type\":\"%s\",\"_id\":\"%s-%lu\"}}\n"
                    "{\"host\":\"%s\""
                    ",\"os\":\"%s\""
                    ",\"app\":\"%s\""
    ...
    sprintf(fingerprint_json[host->thread] + fingerprint_json_len[host->thread], "%s", tmpbuff);
    fingerprint_json_len[host->thread] += strlen(tmpbuff);
#ifndef TESTMODE
    if (fingerprint_json_len[host->thread] + 1024 >= MOLOCH_HTTP_BUFFER_SIZE) {
#endif
#ifdef TESTMODE
        printf("%s\n", fingerprint_json[host->thread]);
#endif
        moloch_http_set(esServer, "/_bulk", 6, fingerprint_json[host->thread], fingerprint_json_len[host->thread], NULL, NULL);
        fingerprint_json_len[host->thread] = 0;
        fingerprint_json[host->thread] = NULL;
#ifndef TESTMODE
    }
#endif
}
