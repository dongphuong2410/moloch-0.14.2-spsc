...
static GOptionEntry entries[] =
{
    ...
    { "emlExtract",     'e',                    0, G_OPTION_ARG_NONE,           &config.emlExtract,         "Turn on eml extract", NULL },
    ...
    { "monitorIdx",  0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_STRING,       &config.pcapMonitorIndex,    "", NULL },
#endif
...
{
#ifdef USE_PCAP_IDX_LIB
    g_free(config.pcapMonitorIndex);
#endif
...
#ifdef USE_PCAP_IDX_LIB
    config.pcapReadOffline = (config.pcapReadFiles || config.pcapReadDirs || config.pcapMonitorIndex);
#else
...
int main(int argc, char **argv)
{
    ...
    cpu_set_t  mask;
    CPU_ZERO(&mask);
    CPU_SET(config.cpuAffinityStart, &mask);
    sched_setaffinity(0, sizeof(mask), &mask);
    LOG("Main thread affinity set to %d\n", config.cpuAffinityStart);
#endif
