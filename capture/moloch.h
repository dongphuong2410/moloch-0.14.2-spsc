...
typedef struct moloch_config {
    ...
    int       cpuAffinityStart;
    uint32_t  maxLimitEnqueue;
#endif
#ifdef USE_PCAP_IDX_LIB
    char     *pcapMonitorIndex;
#endif
} MolochConfig_t;
...
/******************************************************************************/
typedef struct moloch_extract_file {
    /* Common Fields */
    char                    id[1024];
    char                    type[1024];
    unsigned long           time;
    ...
    //char                    path[1024];
    char                    name[1024];
    struct moloch_extract_file *next;
} MolochExtractFile_t;
...
typedef struct host_data {
    ...
    struct host_data    *prev;
    struct host_data    *newer;
    struct host_data    *older;
} MolochFpHost_t;
...
typedef struct {
    ...
    char                in_http;
    char                http_req_done;
    uint32_t            next_cli_seq;
    uint32_t            next_srv_seq;
    MolochFpHost_t      server;
    MolochFpHost_t      client;
    void                *http_tmp;
} MolochFpFlow_t;
...
#ifdef USE_ONE_SSID
MolochSession_t *moloch_session_find_or_create(MolochPacket_t *packet, int *isNew);
#else
