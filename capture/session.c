...
/* sicriops */
    int maxcount;
    maxcount = (DLL_COUNT(cmd_, &sessionCmds[thread]) > 100) ? 100 : DLL_COUNT(cmd_, &sessionCmds[thread]);
...
/* sicriops */
   maxcount = (DLL_COUNT(q_, &closingQ[thread]) > 100) ? 100 : DLL_COUNT(q_, &closingQ[thread]);
   for (count = 0; count < maxcount; count++) {
/* sicriops */
    // Closing Q
    //for (count = 0; count < 100; count++) {
...
/* sicriops */
    maxcount = (DLL_COUNT(q_, &sessionsQ[thread][ses]) > 100) ? 100 : DLL_COUNT(q_, &sessionsQ[thread][ses]);
        //for (count = 0; count < 100; count++) {
/* sicriops */
        for (count = 0; count < maxcount; count++) {
...
/* sicriops */
    maxcount = (DLL_COUNT(tcp_, &tcpWriteQ[thread]) > 100) ? 100 : DLL_COUNT(tcp_, &tcpWriteQ[thread]);
    for (count = 0; count < maxcount; count++) {
/* sicriops */
    //for (count = 0; count < 100; count++) {
