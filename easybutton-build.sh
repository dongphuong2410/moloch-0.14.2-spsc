...
#p0f
P0FDIR=`pwd`/p0f
P0FBUILD="--with-libp0f=$P0FDIR"

#curl_wrapper
CURLWRAPPERDIR=`pwd`/curl_wrapper/src
CURLWRAPPERBUILD="--with-curlwrapper=$CURLWRAPPERDIR"

#ripmime
RIPMIMEDIR=`pwd`/ripmime
RIPMIMEBUILD="--with-libripmime=$RIPMIMEDIR"
...
cd ..
echo "./configure --prefix=$TDIR $PCAPBUILD $P0FBUILD $CURLWRAPPERBUILD $RIPMIMEBUILD --with-yara=thirdparty/yara-$YARA --with-GeoIP=thirdparty/GeoIP-$GEOIP $WITHGLIB --with-curl=thirdparty/curl-$CURL"
./configure --prefix=$TDIR $PCAPBUILD $P0FBUILD $CURLWRAPPERBUILD $RIPMIMEBUILD --with-yara=thirdparty/yara-$YARA --with-GeoIP=thirdparty/GeoIP-$GEOIP $WITHGLIB --with-curl=thirdparty/curl-$CURL
