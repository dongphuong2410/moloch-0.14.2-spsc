...
static inline u64 bloom4_64(p0fhdlr *hdlr, u32 val) {
  u32 hash = hash32(&val, 4, hdlr->hash_seed);
  u64 ret;
  ret = (1LL << (hash & 63));
  ret ^= (1LL << ((hash >> 8) & 63));
  ret ^= (1LL << ((hash >> 16) & 63));
...
static s32 lookup_hdr(p0fhdlr *hdlr, u8* name, u32 len, u8 create) {

  u32  bucket = hash32(name, len, hdlr->hash_seed) % SIG_BUCKETS;

  u32* p = hdlr->hdr_by_hash[bucket];
  u32  i = hdlr->hbh_cnt[bucket];

  while (i--) {
    if (!memcmp(hdlr->hdr_names[*p], name, len) && 
        !hdlr->hdr_names[*p][len]) return *p;
    p++;
  }
  ...
  hdlr->hdr_names = DFL_ck_realloc(hdlr->hdr_names, (hdlr->hdr_cnt + 1) * sizeof(u8*));
  hdlr->hdr_names[hdlr->hdr_cnt] = DFL_ck_memdup_str(name, len);

  hdlr->hdr_by_hash[bucket] = DFL_ck_realloc(hdlr->hdr_by_hash[bucket],
    (hdlr->hbh_cnt[bucket] + 1) * 4);

  hdlr->hdr_by_hash[bucket][hdlr->hbh_cnt[bucket]++] = hdlr->hdr_cnt++;

  return hdlr->hdr_cnt - 1;
...
void p0f_http_init(p0fhdlr *hdlr) {
  u32 i;

  /* Do not change - other code depends on the ordering of first 6 entries. */

  lookup_hdr(hdlr, SLOF("User-Agent"), 1);      /* 0 */
  lookup_hdr(hdlr, SLOF("Server"), 1);          /* 1 */
  lookup_hdr(hdlr, SLOF("Accept-Language"), 1); /* 2 */
  lookup_hdr(hdlr, SLOF("Via"), 1);             /* 3 */
  lookup_hdr(hdlr, SLOF("X-Forwarded-For"), 1); /* 4 */
  lookup_hdr(hdlr, SLOF("Date"), 1);            /* 5 */
...
#define HDR_DAT 5
  ...
  while (req_optional[i].name) {
    req_optional[i].id = lookup_hdr(hdlr, SLOF(req_optional[i].name), 1);
    i++;
  }

  i = 0;
  while (resp_optional[i].name) {
    resp_optional[i].id = lookup_hdr(hdlr, SLOF(resp_optional[i].name), 1);
    i++;
  }

  i = 0;
  while (req_skipval[i].name) {
    req_skipval[i].id = lookup_hdr(hdlr, SLOF(req_skipval[i].name), 1);
    i++;
  }

  i = 0;
  while (resp_skipval[i].name) {
    resp_skipval[i].id = lookup_hdr(hdlr, SLOF(resp_skipval[i].name), 1);
    i++;
  }

  i = 0;
  while (req_common[i].name) {
    req_common[i].id = lookup_hdr(hdlr, SLOF(req_common[i].name), 1);
    i++;
  }

  i = 0;
  while (resp_common[i].name) {
    resp_common[i].id = lookup_hdr(hdlr, SLOF(resp_common[i].name), 1);
    i++;
  }
...
static void http_find_match(p0fhdlr *hdlr, u8 to_srv, struct http_sig* ts, u8 dupe_det) {

  struct http_sig_record* gmatch = NULL;
  struct http_sig_record* ref = hdlr->httpsigs[to_srv];
  u32 cnt = hdlr->http_sig_cnt[to_srv];
  ...
  while (cnt--) {
    ...
    for (rs_hdr = 0; rs_hdr < rs->miss_cnt; rs_hdr++) {
      ...
      u64 miss_bloom4 = bloom4_64(hdlr, rs->miss[rs_hdr]);
...
void http_register_sig(p0fhdlr *hdlr, u8 to_srv, u8 generic, s32 sig_class, u32 sig_name,
  ...
  hdlr->httpsigs[to_srv] = DFL_ck_realloc(hdlr->httpsigs[to_srv], sizeof(struct http_sig_record) *
    (hdlr->http_sig_cnt[to_srv] + 1));

  hrec = &hdlr->httpsigs[to_srv][hdlr->http_sig_cnt[to_srv]];
  ...
  while (*val != ':') {
    ...
    id = lookup_hdr(hdlr, val, nxt - val, 1);

    hsig->hdr[hsig->hdr_cnt].id = id;
    hsig->hdr[hsig->hdr_cnt].optional = optional;

    if (!optional) hsig->hdr_bloom4 |= bloom4_64(hdlr, id);
  ...
  http_find_match(hdlr, to_srv, hsig, 1);
...
void http_parse_ua(p0fhdlr *hdlr, u8* val, u32 line_no) {
  ...
  while (*val) {
    ...
    id = lookup_name_id(hdlr, val, nxt - val);
    ...
    hdlr->ua_map = DFL_ck_realloc(hdlr->ua_map, (hdlr->ua_map_cnt + 1) *
                        sizeof(struct ua_map_record));

    hdlr->ua_map[hdlr->ua_map_cnt].id = id;
   
    if (!name) hdlr->ua_map[hdlr->ua_map_cnt].name = DFL_ck_strdup(hdlr->fp_os_names[id]);
      else hdlr->ua_map[hdlr->ua_map_cnt].name = name;
...
u8* dump_sig(p0fhdlr *hdlr, u8 to_srv, struct http_sig* hsig) {
...
static void score_nat(p0fhdlr *hdlr, u8 to_srv, struct packet_flow* f) {
  ...
  if (to_srv && m->class_id == -1 && f->http_tmp.sw && !f->http_tmp.dishonest) {
    ...
    for (i = 0; i < hdlr->ua_map_cnt; i++)
      if (strstr((char*)f->http_tmp.sw, (char*)hdlr->ua_map[i].name)) break;

    if (i != hdlr->ua_map_cnt) {

      if (hdlr->ua_map[i].id != hd->last_name_id) {
...
static void fingerprint_http(p0fhdlr *hdlr, u8 to_srv, struct packet_flow* f) {

  struct http_sig_record* m;
  u8* lang = NULL;

  http_find_match(hdlr, to_srv, &f->http_tmp, 0);
  m = f->http_tmp.matched;
...
static u8 parse_pairs(p0fhdlr *hdlr, u8 to_srv, struct packet_flow* f, u8 can_get_more) {
  ...
  while ((off = f->http_pos) < plen) {
    ...
    if (pay[off] == '\r' || pay[off] == '\n') {
      ...
      fingerprint_http(hdlr, to_srv, f);
    ...
    hid = lookup_hdr(hdlr, pay + f->http_pos, nlen, 0);
    ...
    if (hid < 0) {
      ...
      f->http_tmp.hdr_bloom4 |= bloom4_64(hdlr, hid);
...
u8 process_http(p0fhdlr *hdlr, u8 to_srv, struct packet_flow* f) {
  ...
  if (to_srv) {
    ...
    return parse_pairs(hdlr, 1, f, can_get_more);
    ...
    return parse_pairs(hdlr, 0, f, can_get_more);
