...
void handle_query(p0fhdlr *hdlr, struct p0f_api_query* q, struct p0f_api_response* r) {
  ...
  switch (q->addr_type) {
    ...
    case P0F_ADDR_IPV4:
    case P0F_ADDR_IPV6:
      h = lookup_host(0, q->addr, q->addr_type); //TODO:hash_seed not transfered
      break;
  ...
  if (h->last_name_id != -1) {
    ...
    strncpy((char*)r->os_name, (char*)hdlr->fp_os_names[h->last_name_id],
            P0F_STR_MAX + 1);
  ...
  if (h->http_name_id != -1) {
    ...
    strncpy((char*)r->http_name, (char*)hdlr->fp_os_names[h->http_name_id],
            P0F_STR_MAX + 1);
