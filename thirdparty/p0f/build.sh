...
BASIC_CFLAGS="-Wall -Wno-format -I/usr/local/include/ \
              -I/opt/local/include/ -I../curl_wrapper/lib/curl-7.37.1/include -I../curl_wrapper/src/ -DVERSION=\"$VERSION\" $CFLAGS"

BASIC_LDFLAGS="-L/usr/local/lib/ -L/opt/local/lib -L../curl_wrapper/lib/curl-7.37.1/lib/.libs/ -lcurl $LDFLAGS"

USE_CFLAGS="-fstack-protector-all -fPIE -fPIC -D_FORTIFY_SOURCE=2 -g -gdwarf-2 \
            $BASIC_CFLAGS"
...
OBJFILES="api.c process.c fp_tcp.c fp_mtu.c fp_http.c readfp.c storefp.c ../curl_wrapper/src/curl_wrapper.c" 
...
if [ "$1" = "clean" -o "$1" = "publish" ]; then
  ...
  rm -f -- "$PROGNAME" *.exe *.a *.o a.out *~ core core.[1-9][0-9]* *.stackdump COMPILER-WARNINGS 2>/dev/null
  ...
  exit 0
    ...
    echo "[+] Configuring and build library"
    BASIC_CFLAGS="$BASIC_CFLAGS -Og"
    USE_CFLAGS="$USE_CFLAGS -Og"

    $CC $USE_CFLAGS -c $"$PROGNAME.c" $OBJFILES $USE_LIBS $USE_LDFLAGS -ggdb -O0
	ar ruvs libp0f.a p0f.o readfp.o storefp.o process.o fp_tcp.o fp_http.o fp_mtu.o api.o
    # Build the unit test for libp0f library
    $CC $USE_CFLAGS test/insertfp.c libp0f.a -o test/insertfp -lpcap $USE_LDFLAGS
	exit 0
...
echo $USE_CFLAGS
#$CC $USE_CFLAGS $USE_LDFLAGS "$TMP.c" -o "$TMP" &>"$TMP.log"
$CC $USE_CFLAGS $USE_LDFLAGS "$TMP.c" -o "$TMP" 
