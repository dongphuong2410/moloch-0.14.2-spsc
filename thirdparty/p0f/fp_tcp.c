...
static void tcp_find_match(p0fhdlr *hdlr, u8 to_srv, struct tcp_sig* ts, u8 dupe_det,
                           u16 syn_mss) {
  ...
  CP(hdlr->tcpsigs[to_srv][bucket]);

  for (i = 0; i < hdlr->tcp_sig_cnt[to_srv][bucket]; i++) {

    struct tcp_sig_record* ref = hdlr->tcpsigs[to_srv][bucket] + i;
...
void tcp_register_sig(p0fhdlr *hdlr, u8 to_srv, u8 generic, s32 sig_class, u32 sig_name,
  ...
  opt_hash = hash32(opt_layout, opt_cnt, hdlr->hash_seed);
  ...
  tcp_find_match(hdlr, to_srv, tsig, 1, 0);
  ...
  hdlr->tcpsigs[to_srv][bucket] = DFL_ck_realloc(hdlr->tcpsigs[to_srv][bucket],
    (hdlr->tcp_sig_cnt[to_srv][bucket] + 1) * sizeof(struct tcp_sig_record));

  trec = hdlr->tcpsigs[to_srv][bucket] + hdlr->tcp_sig_cnt[to_srv][bucket];

  hdlr->tcp_sig_cnt[to_srv][bucket]++;
...
static void packet_to_sig(p0fhdlr *hdlr, struct packet_data* pk, struct tcp_sig* ts) {

  ts->opt_hash = hash32(pk->opt_layout, pk->opt_cnt, hdlr->hash_seed);
...
static void score_nat(p0fhdlr *hdlr, u8 to_srv, struct tcp_sig* sig, struct packet_flow* f) {
...
log_and_update:
  ...
  if (sig->matched) {
    ...
    if (hd->last_class_id != sig->matched->class_id
            || hd->last_name_id != sig->matched->name_id
            || (!hd->last_flavor && sig->matched->flavor)
            || (hd->last_flavor && !sig->matched->flavor)
            || (hd->last_flavor && sig->matched->flavor && strcmp((const char *)hd->last_flavor, (const char *)sig->matched->flavor))) {
        store_fp_write_fp(hdlr, (const u8 *)addr_to_str(hd->addr, hd->ip_ver), (const u8 *)hdlr->fp_os_names[sig->matched->name_id], (const u8 *)sig->matched->flavor);
    }
...
struct tcp_sig* fingerprint_tcp(p0fhdlr *hdlr, u8 to_srv, struct packet_data* pk,
  ...
  tcp_find_match(hdlr, to_srv, sig, 0, f->syn_mss);
