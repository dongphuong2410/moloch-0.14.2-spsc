...
//static s32         api_fd = -1;                 /* API socket descriptor              */
...
static void get_hash_seed(p0fhdlr *hdlr) {
  ...
  if (read(f, &(hdlr->hash_seed), sizeof(hdlr->hash_seed)) != sizeof(hdlr->hash_seed))
...
{
    p0fhdlr *hdlr = DFL_ck_alloc(sizeof(p0fhdlr));
	//close_spare_fds();
	get_hash_seed(hdlr);
	p0f_http_init(hdlr);
	read_config(hdlr, fp_file ? fp_file : (u8*)FP_FILE);
    store_fp_init(hdlr);
    return hdlr;
}
...
void p0f_close(p0fhdlr *hdlr)
{
    if (hdlr) {
        free_config(hdlr);
        destroy_all_hosts(hdlr);
        store_fp_destroy(hdlr);
        DFL_ck_free(hdlr);
    }
}
