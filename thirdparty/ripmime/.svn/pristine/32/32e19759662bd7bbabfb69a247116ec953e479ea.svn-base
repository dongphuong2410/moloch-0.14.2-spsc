...
#define RT32( to, num_addr, offset ) if (read_32(to, num_addr, offset)==-1) return -1
...
{
	snprintf( to->path, TNEF_PATH_SIZE , "%s", path);
...
Errors:
------------------------------------------------------------------------*/
int read_32( struct TNEF_object *to, uint32 *value, uint8 *tsp)
{
	uint8 a,b,c,d;

	if ((tsp +4) > to->tnef_limit)
	{
...
Errors:
------------------------------------------------------------------------*/
int read_16( struct TNEF_object *to, uint16 *value, uint8 *tsp)
{
	uint8 a,b;
...
Errors:
------------------------------------------------------------------------*/
int save_attach_data(struct TNEF_object *to, char *title, uint8 *tsp, uint32 size)
{
	FILE *out;
	char filename[1024];
	size_t bc;

	snprintf(filename, sizeof(filename),"%s/%s", to->path, title );
...
Errors:
------------------------------------------------------------------------*/
int handle_props(struct TNEF_object *to, uint8 *tsp)
{
	int bytes = 0;
	uint32 num_props = 0;
	uint32 x = 0;
	...
	while (x < num_props)
	{
		...
		RT32(to, &prop_tag, tsp+bytes);
		bytes += sizeof(prop_tag);

		switch (prop_tag & PROP_TYPE_MASK)
		{
			case PT_BINARY:
				RT32(to, &num, tsp+bytes);
				bytes += sizeof(num);
				RT32(to, &num, tsp+bytes);
				bytes += sizeof(num);
				if (prop_tag == PR_RTF_COMPRESSED)
				{
					sprintf (filename, "XAM_%d.rtf", to->file_num);
					to->file_num++;
					save_attach_data(to, filename, tsp+bytes, num);
				}
				/* num + PAD */
...
Errors:
------------------------------------------------------------------------*/
int read_attribute(struct TNEF_object *to, uint8 *tsp)
{
...
//	uint8 component = 0;
	...
	rv = read_32(to, &attribute, tsp+bytes);
	if (rv == -1) return -1;
	...
	switch (attribute) {
		...
		case attAttachData:
			...
			if (TNEF_glb.savedata && strlen(attach_title)>0 && attach_size > 0) {
				if (!save_attach_data(to, attach_title, (uint8 *)attach_loc,attach_size))
				{
					if (TNEF_VERBOSE) {
		...
		case attAttachRenddata:
			...
			break;
		case attMAPIProps:
			if (handle_props(to, tsp+header)==-1) return -1;
			break;
		case attRecipTable:
			default_handler(attribute, tsp+header, size);
			break;
		...
		case attTnefVersion:
			{
				uint32 version;
				rv = read_32(to, &version, tsp+header);
				if (rv == -1) return -1;
			}
			break;
		case attOemCodepage:
...
Errors:
------------------------------------------------------------------------*/
int TNEF_decode_tnef(struct TNEF_object *to, uint8 *tnef_stream, int size)
{
	...
	//
	ra_response = read_32(to, &tnefs, tsp);
	...
	if (read_16(to, &tnef_attachkey, tsp) == -1) return -1;
	...
	while ((tsp - tnef_stream) < size)
	{
		...
		ra_response = read_attribute(to, tsp);
		if ( ra_response > 0 )
		{
			tsp += ra_response;
		} else {
...
int TNEF_main( struct TNEF_object *to, char *filename )
{
	...
	//
	to->tnef_home = tnef_stream = (uint8 *)malloc(size);
	to->tnef_limit = to->tnef_home +size;
	...
	if (tnef_stream == NULL)
	{
		...
		if (to->tnef_home) free(to->tnef_home);
		return -1;
	}
	...
	//
	TNEF_decode_tnef(to, tnef_stream,size);
