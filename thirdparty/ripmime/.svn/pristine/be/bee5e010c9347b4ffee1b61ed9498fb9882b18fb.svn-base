...
int MIMEH_object_init(struct MIMEH_object *ho)
{
	ho->doubleCR = 0;
	ho->headerline = NULL;
	ho->doubleCR_save = 1;
	ho->doubleCRname[0]='\0';
	ho->appledouble_filename[0]='\0';
	ho->headerline_original = NULL;
	ho->doubleCR_count = 0;
	ho->header_longsearch=0;
	ho->output_dir[0]='\0';
	ho->header_file = NULL;
	ho->original_header_file = NULL;
	return 0;
}
...
\------------------------------------------------------------------*/
int MIMEH_get_doubleCR( struct MIMEH_object *ho )
{
	return ho->doubleCR;
}
...
\------------------------------------------------------------------*/
int MIMEH_set_doubleCR(struct MIMEH_object *ho, int level )
{
	ho->doubleCR = level;
...
\------------------------------------------------------------------*/
int MIMEH_set_doubleCR_save( struct MIMEH_object *ho, int level )
{
	ho->doubleCR_save = level;

	return ho->doubleCR_save;
}
...
\------------------------------------------------------------------*/
int MIMEH_get_doubleCR_save( struct MIMEH_object *ho )
{
	return ho->doubleCR_save;
}
...
\------------------------------------------------------------------*/
char *MIMEH_get_doubleCR_name( struct MIMEH_object *ho )
{
	return ho->doubleCRname;
}
...
\------------------------------------------------------------------*/
int MIMEH_set_outputdir( struct MIMEH_object *ho, char *dir )
{
	if (dir) snprintf(ho->output_dir,_MIMEH_STRLEN_MAX,"%s",dir);
	return 0;
}
...
Errors:
------------------------------------------------------------------------*/
int MIMEH_set_headers_save( struct MIMEH_object *ho, FILE *f )
{
	ho->header_file = f;
	glb.save_headers = 1;
	return 0;
...
\------------------------------------------------------------------*/
int MIMEH_set_headers_original_save_to_file( struct MIMEH_object *ho, FILE *f )
{
	if (f == NULL) glb.original_header_save_to_file = 0;
	else glb.original_header_save_to_file = 1;
	ho->original_header_file = f;
...
\------------------------------------------------------------------*/
int MIMEH_set_headers_nosave( struct MIMEH_object *ho )
{
	ho->header_file = NULL;
	glb.save_headers = 0;
	return 0;
...
\------------------------------------------------------------------*/
char *MIMEH_get_headers_ptr( struct MIMEH_object *ho )
{
	return ho->headerline;
}
...
\------------------------------------------------------------------*/
char *MIMEH_get_headers_original_ptr( struct MIMEH_object *ho )
{
	return ho->headerline_original;
}
...
\------------------------------------------------------------------*/
int MIMEH_set_header_longsearch( struct MIMEH_object *ho, int level )
{
	ho->header_longsearch = level;

	return ho->header_longsearch;
}
...
\------------------------------------------------------------------*/
int MIMEH_save_doubleCR( struct MIMEH_object *ho, FFGET_FILE *f )
{
	//char c;
	int c;
	FILE *fo;
	...
	do {
		ho->doubleCR_count++;
		snprintf(ho->doubleCRname,_MIMEH_STRLEN_MAX,"%s/doubleCR.%d",ho->output_dir,ho->doubleCR_count);
	}
	while (stat(ho->doubleCRname, &st) == 0);


	fo = fopen(ho->doubleCRname,"w");
	if (!fo)
	{
		LOGGER_log("%s:%d:MIMEH_save_doubleCR:ERROR: unable to open '%s' to write (%s)", FL,ho->doubleCRname,strerror(errno));
		return -1;
	}
...
Errors:
------------------------------------------------------------------------*/
int MIMEH_read_headers( struct MIMEH_object *ho, FFGET_FILE *f )
{
	...
	**/
	...
	do {
		...
		while ((fget_result=FFGET_fgets(buffer,_MIMEH_STRLEN_MAX, f)))
		{
			...
			if (MIMEH_DNORMAL)LOGGER_log("%s:%d:MIMEH_read_headers: Data In=[sz=%d:tb=%d:mem=%p]'%s'",FL, linesize, f->trueblank, ho->headerline, buffer);

			// If we are being told to copy the input data to an output file
			//		then do so here (this is for the originals)
			if ((glb.original_header_save_to_file > 0)&&(ho->original_header_file != NULL))
			{
				if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: saving to file...",FL);
				fprintf(ho->original_header_file,"%s",linestart);
			}
			...
			if (glb.save_headers_original > 0)
			{
				...
				tmp_original = realloc(ho->headerline_original, totalsize_original+linesize+1);
				if (tmp_original == NULL)
				{
					...
					if (ho->headerline_original != NULL) free(ho->headerline_original);
					ho->headerline_original = NULL;
					return -1;
				}

				if (ho->headerline_original == NULL)
				{
					ho->headerline_original = tmp_original;
					totalsize_original = linesize +1;
					PLD_strncpy( ho->headerline_original, linestart, (linesize+1));
					if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: '%s'", FL, ho->headerline_original);
				} else {
					ho->headerline_original = tmp_original;
					PLD_strncpy( (ho->headerline_original +totalsize_original -1), linestart, (linesize +1));
					totalsize_original += linesize;
					if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: HO =  '%s'", FL, ho->headerline_original);
				}
			...
			if (ho->headerline == NULL)
			{
				...
				ho->headerline = tmp;
				totalsize = linesize;
				PLD_strncpy(ho->headerline, linestart, (linesize +1));
				headerline_end = ho->headerline +totalsize;
			...
			{
				if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: Appending of new data to existing header  existing-headerline = %p  new realloc block = %p linestart = %p linesize = %d",FL, ho->headerline, tmp, linestart, linesize); 
				...
				if ((linestart < lineend)&&((*linestart == '\t')||(*linestart == ' ')))
				{
					...
					p = ho->headerline +totalsize -1;
					if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: unwrapping headers headers=%p, p = %p",FL,ho->headerline, p);
					while ((p >= ho->headerline)&&(( *p == '\n' )||( *p == '\r' )))
					{
				...
				if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_read_headers:DEBUG: Memcopying line, source = %p, dest = %p, size = %d", FL, linestart, ho->headerline +totalsize, linesize);
				memcpy((ho->headerline +totalsize), linestart, (linesize));
				totalsize += linesize;
				*(ho->headerline +totalsize) = '\0';


			}	// If the ho->headerline already is allocated and we're appending to it.
			...
			if (f->trueblank)
			{
				...
				MIMEH_fix_header_mistakes( ho->headerline );
				MDECODE_decode_ISO( ho->mdo, ho->headerline, totalsize  );

				if ((glb.save_headers)&&(ho->headerline))
				{
					if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIME_read_headers:DEBUG: Saving header line.",FL);
					fprintf(ho->header_file,"%s",ho->headerline);
				}
			...
			if (FFGET_doubleCR)
			{
				if (ho->doubleCR_save != 0)
				{
					MIMEH_save_doubleCR(ho, f);
					ho->doubleCR = 1;
				}
				FFGET_doubleCR = 0;
				FFGET_SDL_MODE = 0;
		...
		if (!fget_result)
		{
			...
			if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIME_read_headers:DEBUG: save_headers=%d totalsize=%d headerline=%s", FL, glb.save_headers, totalsize, ho->headerline);
			...
			if (ho->header_longsearch > 0) {
				/** Test the headers for RFC compliance... **/
				is_RFC822_headers =  MIMEH_are_headers_RFC822(ho->headerline);
	...
	} while ((is_RFC822_headers==0)&&(ho->header_longsearch>0)&&(result==0)&&(search_count<glb.longsearch_limit));
...
\------------------------------------------------------------------*/
int MIMEH_parse_contenttype( struct MIMEH_object *ho, char *header_name, char *header_value, struct MIMEH_header_info *hinfo )
{
...
/** 20041216-1106:PLD: Increase our sanity **/
		...
		else if (strstr(q,"application/applefile")) 
		{
			...
			{
				if (strlen(ho->appledouble_filename)>0)
				{
					snprintf(hinfo->filename, sizeof(hinfo->filename), "%s.applemeta", ho->appledouble_filename );
				} else {
...
int MIMEH_parse_contenttransferencoding( char *header_name, char *header_value, struct MIMEH_header_info *hinfo )
{
 ...
 /** 20041216-1107:PLD: Increase our sanity **/
		...
		if (strstr(p,"base64") || strstr(p, "base-64"))
		{
...
\------------------------------------------------------------------*/
int MIMEH_parse_contentdisposition( struct MIMEH_object *ho, char *header_name, char *header_value, struct MIMEH_header_info *hinfo )
{
	char  *p;
	char *hv = strdup(header_value);
...
/** 20041216-1107:PLD: Increase our sanity **/
		...
		if (p != NULL)
		{
			...
			{
				snprintf( ho->appledouble_filename, sizeof(ho->appledouble_filename), "%s", hinfo->filename );	
				if (MIMEH_DNORMAL) LOGGER_log("%s:%d:MIMEH_parse_contentdisposition:DEBUG: Setting appledouble filename to: '%s'",FL,ho->appledouble_filename);
			}
...
\------------------------------------------------------------------*/
int MIMEH_parse_subject( struct MIMEH_object *ho, char *header_name, char *header_value, struct MIMEH_header_info *hinfo )
{
	int result = 0;
	result = MIMEH_parse_generic( header_name, header_value, hinfo, "subject", hinfo->subject, sizeof(hinfo->subject)  );
	snprintf(ho->subject, sizeof(ho->subject),"%s", hinfo->subject);
...
\------------------------------------------------------------------*/
int MIMEH_headers_process( struct MIMEH_object *ho, struct MIMEH_header_info *hinfo, char *headers )
{
		...
		{ 
			...
			if ( header_value_end_position != NULL )
			{
				...
				*header_value_end_position = '\0';
				...
				PLD_strlower( header_name );
				MIMEH_parse_subject( ho, header_name, header_value, hinfo );
				MIMEH_parse_contenttype( ho, header_name, header_value, hinfo );
				MIMEH_parse_contenttransferencoding( header_name, header_value, hinfo );
				MIMEH_parse_contentdisposition( ho, header_name, header_value, hinfo );
...
\------------------------------------------------------------------*/
int MIMEH_headers_get( struct MIMEH_object *ho, struct MIMEH_header_info *hinfo, FFGET_FILE *f )
{
	int result = 0;
	...
	result = MIMEH_read_headers(ho, f);
	...
	if (result == -1)
	{
		if (ho->headerline) free(ho->headerline);
		return result;
	}
...
\------------------------------------------------------------------*/
int MIMEH_headers_cleanup( struct MIMEH_object *ho )
{
	if (ho->headerline != NULL)
	{
		free(ho->headerline);
...
\------------------------------------------------------------------*/
int MIMEH_parse_headers( struct MIMEH_object *ho, FFGET_FILE *f, struct MIMEH_header_info *hinfo )
{
	int result = 0;
	...
	if ( result == 0 ) result = MIMEH_headers_get( ho, hinfo, f );
	DMIMEH LOGGER_log("%s:%d:MIMEH_parse_headers:DEBUG: Processing headers",FL);
	if ( result == 0 ) result = MIMEH_headers_process( ho, hinfo, ho->headerline );
	DMIMEH LOGGER_log("%s:%d:MIMEH_parse_headers:DEBUG: Cleanup of headers",FL);
	if ( result == 0 ) result = MIMEH_headers_cleanup(ho); 
