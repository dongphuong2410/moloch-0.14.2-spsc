...
static void iconvert(struct MDECODE_object *mdo, char *text)
{
  if ((glb.out_charset[0] == 0) || (mdo->current_charset[0] == 0)
      || (strcmp(glb.out_charset, mdo->current_charset) == 0))
    return;
  if (!strcmp(mdo->current_charset, "ks_c_5601-1987")) {
	strncpy(mdo->current_charset, "EUC-KR", strlen("EUC-KR") + 1);
  }

  iconv_t _iconv = iconv_open(glb.out_charset, mdo->current_charset);
  if ((iconv_t) -1 != _iconv) {
    size_t inbytesleft = strlen(text);
    size_t outbytesleft = inbytesleft * 2 + 1;
    char *out_str = malloc(outbytesleft);
    ...
    *out_str_pp = '\0';
    snprintf(text, strlen(out_str), "%s", out_str);
    iconv_close(_iconv);
    free(out_str);
  }
}
...
//		if ( start_pair ) end_pair   = strstr( start_pair +2, "?=" );
		...
		if (( start_pair != NULL ))
		{
			...
			if ( (iso_start) && (iso_end) )
			{
				...
				snprintf( mdo->current_charset, sizeof(mdo->current_charset), "%s", iso);
				DMD LOGGER_log("%s:%d:MDECODE_decode_ISO:DEBUG: ISO char set = '%s'",FL,mdo->current_charset);
