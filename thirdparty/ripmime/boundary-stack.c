...
{
	bo->count = 0;
	bo->smallest_length = -1;
	bo->have_empty_boundary = 0;
	bo->boundarystack = NULL;
	return 0;
}
/*-----------------------------------------------------------------\
...
int BS_clear( struct BS_object *bo )
{
	...
	while (bo->boundarystack)
	{
		DBS LOGGER_log("%s:%d:BS_clear:DEBUG: Popping off %p",FL,bo->boundarystack);
		next = bo->boundarystack->next;
		free(bo->boundarystack->boundary);
		free(bo->boundarystack);
		bo->boundarystack = next;
	}
...
int BS_push( struct BS_object *bo, char *boundary )
{
	...
	if ((glb.hold_limit > 0)&&(bo->count >= glb.hold_limit)) 
	{
	...
	DBS LOGGER_log("%s:%d:BS_push:DEBUG: head = %p, nn = %p boundary = '%s'",FL, bo->boundarystack, node, boundary);

	if (node)
	{
		node->next = bo->boundarystack;
		bo->boundarystack = node;
		bo->boundarystack->boundary = strdup(boundary);
		bo->boundarystack->boundary_length = strlen(bo->boundarystack->boundary);
		if (bo->boundarystack->boundary_length == 0) bo->have_empty_boundary = 1;
		bo->boundarystack->boundary_nhl = BS_non_hyphen_length(boundary);
		bo->count++;

		// Set the smallest length
		if (bo->smallest_length == -1) bo->smallest_length = bo->boundarystack->boundary_length;
		else if (bo->boundarystack->boundary_length < bo->smallest_length) bo->smallest_length = bo->boundarystack->boundary_length;

		DBS LOGGER_log("%s:%d:DEBUGX: smallest = %d, NHL = %d,  boundary = '%s'",FL,bo->smallest_length, node->boundary_nhl, boundary);
	}
	else
	    {
...
char *BS_pop( struct BS_object *bo )
{
	...
	struct BS_node *node = bo->boundarystack;

	if (bo->boundarystack)
	{
		bo->boundarystack = bo->boundarystack->next;
		PLD_strncpy(bo->boundarystacksafe,node->boundary, BS_STRLEN_MAX);
		free(node->boundary);
		free(node);
		bo->count--;
	}

	return bo->boundarystacksafe;
}
...
char *BS_top( struct BS_object *bo )
{
	...
	if (bo->boundarystack)
	{
		return bo->boundarystack->boundary;
	}
	else return NULL;
}
...
\------------------------------------------------------------------*/
int BS_is_long_enough( struct BS_object *bo, int blen )
{
	if (bo->smallest_length == -1) return 0;
	if (blen >= bo->smallest_length) return 1;
...
\------------------------------------------------------------------*/
int BS_boundary_detect( struct BS_object *bo, char *haystack, char *needle, int needle_length )
{
	int result=1;
	int current_start = glb.detect_limit;
	char *haystack_start;

	if ((bo->have_empty_boundary == 1)&&(needle_length < 1))
	{
	...
	if ((needle_length < 1)&&(bo->have_empty_boundary == 0)) return 1;
...
int BS_cmp( struct BS_object *bo, char *boundary, int len )
{
	...
	if ((!boundary)||(bo->count == 0)) return 0;
	//if ((glb.smallest_length > 0)&&(len < glb.smallest_length)) return 0;
	if (BS_is_long_enough(bo, len) == 0) return 0;
...
//		if (node->boundary_length <= len)
		...
		{	
			...
			{
				if ((BS_boundary_detect(bo, testspace, node->boundary, node->boundary_length))==0)
				{
	...
	if(spin==0)
	{
		...
		if (node != bo->boundarystack)
		{
			nodetmp = bo->boundarystack;
