...
struct MIME_object {
	...
	struct MIMEH_object *ho;
	struct UUENCODE_object *uo;
	struct TNEF_object *to;
};
...
int MIME_unpack_stage2( FFGET_FILE *f, struct MIME_object *, struct MIMEH_header_info *hinfo, int current_recursion_level, struct SS_object *ss );
int MIME_unpack_single(struct MIME_object *mo, char *mpname, int current_recursion_level, struct SS_object *ss );
int MIME_unpack_single_fp(struct MIME_object *mo, FILE *fi, int current_recursion_level, struct SS_object *ss );
int MIME_unpack_mailbox(struct MIME_object *mo, char *mpname, int current_recursion_level, struct SS_object *ss );
int MIME_handle_multipart( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *h, int current_recursion_level, struct SS_object *ss );
int MIME_handle_rfc822( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *h, int current_recursion_level, struct SS_object *ss );
static int _copy_ss(struct SS_object *ss, struct email_attach_file **output);
...
\------------------------------------------------------------------*/
//int MIME_set_decode_doubleCR( struct MIMEH_object *ho, int level )
//{
//	MIMEH_set_doubleCR_save(ho, level); /** 20041106-0859:PLD: Was '0' **/
//
...
Errors:
------------------------------------------------------------------------*/
int MIME_decode_TNEF(struct MIME_object *mo, struct MIMEH_header_info *hinfo, int keep )
{
	int result=0;
	char fullpath[1024];

	snprintf(fullpath,sizeof(fullpath),"%s/%s",mo->unpackdir,hinfo->filename);

	TNEF_set_path(mo->to, mo->unpackdir);

	result = TNEF_main( mo->to, fullpath );
...
Errors:
------------------------------------------------------------------------*/
int MIME_decode_raw( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo, int keep )
{
	int result = 0;
	char fullpath[1024];
	int bufsize=1024;
	...
	while ((readcount=FFGET_raw(f, (unsigned char *) buffer,bufsize)) > 0)
	{
		...
		if (BS_cmp(mo->ho->bo, buffer, readcount))
		{
	...
	if (file_has_uuencode)
	{
		...
		snprintf(full_decode_path,sizeof(full_decode_path),"%s/%s",mo->unpackdir,hinfo->filename);
		...
		result = UUENCODE_decode_uu(NULL, mo->unpackdir, mo->uo, full_decode_path, hinfo->uudec_name, sizeof(hinfo->uudec_name), decode_entire_file, keep );
		...
		if (strlen(hinfo->uudec_name))
		{
			if (strcasecmp(hinfo->uudec_name,"winmail.dat")==0)
			{
				...
				MIME_decode_TNEF( mo, hinfo, keep);
			}
...
Input:         mo: MIME_object struct
hinfo : struct containing information from the last parsed headers
keep : if set, retain the file
Output:
Errors:
------------------------------------------------------------------------*/
int MIME_decode_text( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo, int keep, int ignore)
{
	...
	snprintf(fullfilename,sizeof(fullfilename),"%s/%s",mo->unpackdir,hinfo->filename);
	...
	if (f)
	{
		...
		while ((get_result = FFGET_fgets(line,1023,f)))
		{
			...
			if (line[0] == '-')
			{
				...
				if ((BS_count(mo->ho->bo) > 0)&&(BS_cmp(mo->ho->bo, line,line_len)))
				{
	...
	if (file_has_uuencode)
	{
		...
		snprintf(ffname,256,"%s/%s", mo->unpackdir, hinfo->filename);
		...
		result = UUENCODE_decode_uu( NULL, mo->unpackdir, mo->uo, ffname, hinfo->uudec_name, sizeof(hinfo->uudec_name), 1, keep );
		if (result == -1)
		{
			switch (uuencode_error) {
				case UUENCODE_STATUS_SHORT_FILE:
		...
		if ( result > 0 ) { mo->attachment_count += result; result = 0; }
...
int iglore : Ignore the file (not extract), Xplico don't need the text file extracted from the subject and content
Output:
Errors:
------------------------------------------------------------------------*/
int MIME_decode_64( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo)
{
	...
	if (glb.unique_names) MIME_test_uniquename( mo->unpackdir, hinfo->filename, glb.rename_method );
	snprintf(fullMIME_filename,_MIME_STRLEN_MAX,"%s/%s",mo->unpackdir,hinfo->filename);
		...
		for (i = 0; i < 4; i++)
		{
				...
				//&&(lastchar_was_linebreak == 1))
			{
				...
				if (p == NULL) {
					...
					FFGET_fgets(hinfo->scratch,sizeof(hinfo->scratch), f);

					if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_decode_64:DEBUG: Scratch = '%s'", FL, hinfo->scratch);
					hit = BS_cmp(mo->ho->bo, hinfo->scratch,strlen(hinfo->scratch) +1);
					if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_decode_64:DEBUG: Boundary hit = %d", FL, hit);

				} else {

					*p = '\0';
					hit = BS_cmp(mo->ho->bo, (f->startpoint -1),strlen(f->startpoint) +1);
					*p = '\n';
				}
...
\------------------------------------------------------------------*/
int MIME_decode_64_cleanup( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo)
{
	int result = 0;
	char buffer[128];
	...
	while (FFGET_fgets(buffer, sizeof(buffer), f))
	{
		...
		if (BS_cmp(mo->ho->bo, buffer,strlen(buffer)) > 0) break;
	}
...
struct MIME_object *mo : Information of the unpacking file
...
Errors:
------------------------------------------------------------------------*/
int MIME_doubleCR_decode( struct MIME_object *mo, struct MIMEH_header_info *hinfo, int current_recursion_level )
{
	int result = 0;
	...
	if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_doubleCR_decode:DEBUG: filename=%s, path=%s, recursion=%d", FL, mo->filename, mo->unpackdir, current_recursion_level );
	...
	if (MIME_is_file_RFC822(mo->filename))
	{
		if (MIME_VERBOSE) LOGGER_log("Attempting to decode Double-CR delimeted MIME attachment '%s'\n",mo->filename);
		result = MIME_unpack(NULL, mo->filename, current_recursion_level, NULL); // 20040305-1303:PLD - Capture the result of the unpack and propagate up
	}
	else if (UUENCODE_is_file_uuencoded(h.filename))
	{
		...
		UUENCODE_set_doubleCR_mode(mo->uo, 1);
		result = UUENCODE_decode_uu(NULL, mo->unpackdir, mo->uo, mo->filename, h.uudec_name, _MIMEH_FILENAMELEN_MAX , 1, 1 );
		UUENCODE_set_doubleCR_mode(mo->uo, 0);
		mo->attachment_count += result;
		result = 0;
	}
...
MIME decoding.
Input: 
Output: struct MIME_object
Errors:
------------------------------------------------------------------------*/
struct MIME_object *MIME_init(void)
{
	struct MIME_object *mo = (struct MIME_object *)malloc(sizeof(struct MIME_object));
	if (!mo) goto err;
	mo->filecount = 0;

	mo->ho = (struct MIMEH_object *)malloc(sizeof(struct MIMEH_object));
	if (!mo->ho) goto err1;
	MIMEH_object_init(mo->ho);

	mo->ho->bo = (struct BS_object *)malloc(sizeof(struct BS_object));
	if (!mo->ho->bo) goto err2;
	BS_object_init(mo->ho->bo);

	mo->uo = (struct UUENCODE_object *)malloc(sizeof(struct UUENCODE_object *));
	if (!mo->uo) goto err3;
	UUENCODE_object_init(mo->uo);

	mo->ho->mdo = (struct MDECODE_object *)malloc(sizeof(struct MDECODE_object *));
	if (!mo->ho->mdo) goto err4;
	MDECODE_object_init(mo->ho->mdo);

	mo->to = (struct TNEF_object *)malloc(sizeof(struct TNEF_object));
	if (!mo->to) goto err5;
	TNEF_object_init(mo->to);
	return mo;
err5:
	free(mo->ho->mdo);
err4:
...
void MIME_set_unpackdir(struct MIME_object *mo, char *unpackdir)
{
	mo->unpackdir = unpackdir;
	mo->header_defect_count = 0;
	mo->attachment_count = 0;
	mo->subject[0]='\0';
}

/*------------------------------------------------------------------------
Procedure:     MIME_global_init ID:1
Purpose:       Initialise various global variables required parameters to ensure a clean starting of
MIME decoding.
Input: 
Output: 
Errors:
------------------------------------------------------------------------*/
int MIME_global_init(void)
{
...
Errors:
------------------------------------------------------------------------*/
int MIME_decode_encoding( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo, struct SS_object *ss )
{
	int keep = 1;
	int result = -1;
	...
	if (hinfo->filename[0] == '\0')
	{
		if (glb.name_by_type == 1)
		{
			...
			if (filename_prefix != NULL)
			{
				...
				snprintf( hinfo->filename, _MIMEH_FILENAMELEN_MAX, "%s%s%d", glb.blankfileprefix_expliticly_set?glb.blankfileprefix:"", filename_prefix, mo->filecount );

				free(filename_prefix);

			} else {
				snprintf( hinfo->filename, _MIMEH_FILENAMELEN_MAX, "%s%d", glb.blankfileprefix, mo->filecount );
			}
		} else {
	...
	char ori_name[_MIME_STRLEN_MAX];
	snprintf(ori_name, _MIME_STRLEN_MAX, "%s", hinfo->filename);
	if ((glb.unique_names)&&(keep))
	{
		MIME_test_uniquename( mo->unpackdir, hinfo->filename, glb.rename_method );
	}
	...
	// Variable used to ignore textfile (file that is not from attachment, because Xplico don't need this)
	int ignore = 0;
	if (hinfo->content_disposition != _CDISPOSITION_ATTACHMENT) {
		ignore = 1;
	}
	...
	if (1)
	{
		...
		//Contain fname logs to return to Xplico : including
		//original filename and path, seperated by # character
		char fname_log[_MIME_LONG_STRLEN_MAX];
		snprintf(fname_log, _MIME_LONG_STRLEN_MAX, "%s#%s/%s", fp, mo->unpackdir, hinfo->filename);
		...
		if (!ignore)
			SS_push(ss, fname_log, strnlen(fname_log, _MIME_LONG_STRLEN_MAX));
	}
	...
	switch (hinfo->content_transfer_encoding)
	{
		case _CTRANS_ENCODING_B64:
			...
			result = MIME_decode_64(f, mo, hinfo);
			...
			switch (result) {
				...
				case MIME_BASE64_STATUS_HIT_BOUNDARY:
					...
					break;
				case 0:
					result = MIME_decode_64_cleanup(f, mo, hinfo);
					break;
				default:
					break;
			}
		...
		case _CTRANS_ENCODING_7BIT:
			...
			result = MIME_decode_text(f, mo, hinfo, keep, ignore);
			break;
		...
		case _CTRANS_ENCODING_RAW:
			...
			result = MIME_decode_raw(f, mo, hinfo, keep);
			break;
		...
		case _CTRANS_ENCODING_UUENCODE:
			...
			result = UUENCODE_decode_uu(f, mo->unpackdir, mo->uo, hinfo->filename, hinfo->uudec_name, sizeof(hinfo->uudec_name), 0, keep );
			mo->attachment_count += result;
		...
		case _CTRANS_ENCODING_UNSPECIFIED:
			...
			if (1)
			{
				snprintf(hinfo->scratch,sizeof(hinfo->scratch),"%s/%s", mo->unpackdir,hinfo->filename);
				...
				if (MIME_is_file_RFC822(hinfo->scratch) > 0 )
				{
					...
					result = MIME_unpack_single(mo, hinfo->scratch, (hinfo->current_recursion_level+1),ss );
				}
			}
			break;
	...
	if ((result != -1)&&(result != MIME_STATUS_ZERO_FILE))
	{
		...
		if (glb.decode_ole > 0)
		{
			MIME_decode_OLE( mo->unpackdir, hinfo, 0 );
		}
#endif
	...
	MIME_generate_multiple_hardlink_filenames(hinfo,mo->unpackdir);
...
int MIME_handle_multipart( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *h, int current_recursion_level, struct SS_object *ss )
{
	...
	if (( h->content_transfer_encoding != _CTRANS_ENCODING_B64)&&( h->filename[0] == '\0' ))
	{
		...
		result = MIME_unpack_stage2(f, mo, h, current_recursion_level , ss);
		...
		result = MIME_decode_encoding( f, mo, h, ss );
		if (result == 0)
		{
			...
			snprintf(h->scratch,sizeof(h->scratch),"%s/%s", mo->unpackdir, h->filename);
			snprintf(h->filename,sizeof(h->filename),"%s",h->scratch);

			//result = MIME_unpack_single( unpackdir, h->filename, current_recursion_level +1, ss);
			result = MIME_unpack_single(mo, h->filename, current_recursion_level, ss );
		}
...
int MIME_handle_rfc822( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *h, int current_recursion_level, struct SS_object *ss )
{
	...
	if (( h->content_transfer_encoding != _CTRANS_ENCODING_B64)&&( h->filename[0] == '\0' )) 
	{
		...
		result = MIME_decode_encoding( f, NULL, h, ss );
		...
		if (result == 0) {
			...
			if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_handle_rfc822:DEBUG: Now attempting to extract contents of '%s'",FL,h->scratch);
			result = MIME_unpack_single( mo, h->scratch, current_recursion_level, ss );
			result = 0;
		}
...
\------------------------------------------------------------------*/
int MIME_handle_plain( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *h, int current_recursion_level, struct SS_object *ss )
{
	...
	result = MIME_decode_encoding( f, mo, h, ss );
	if (result == MIME_ERROR_FFGET_EMPTY)
	{
		...
		if (MIME_is_file_RFC822(h->scratch)==1)
		{
			...
			if (glb.header_longsearch != 0) MIMEH_set_header_longsearch(mo->ho, glb.header_longsearch);
			result = MIME_unpack_single(mo, h->scratch, current_recursion_level, ss );
			if (glb.header_longsearch != 0) MIMEH_set_header_longsearch(mo->ho, 0);
		}

    MIME_convert_charset(mo->unpackdir, h);
	}
...
Errors:
------------------------------------------------------------------------*/
int MIME_unpack_stage2( FFGET_FILE *f, struct MIME_object *mo, struct MIMEH_header_info *hinfo, int current_recursion_level, struct SS_object *ss )
{
	int result = 0;
	struct MIMEH_header_info *h;
	char *p;
	...
	do {
		...
		result = MIMEH_parse_headers(mo->ho, f,h);
	...
	mo->header_defect_count += MIMEH_get_defect_count(h);
	...
	if ((strlen(mo->subject) < 1)&&(h->subject != NULL)&&(strlen(h->subject) > 0))
	{
		snprintf(mo->subject, sizeof(mo->subject), "%s", h->subject );
	}
	...
	//		flag.
	if (MIMEH_get_doubleCR(mo->ho) != 0)
	{
		mo->filename = MIMEH_get_doubleCR_name(mo->ho);
		MIME_doubleCR_decode( mo, h, current_recursion_level);
		MIMEH_set_doubleCR( mo->ho, 0 );
		FFGET_SDL_MODE = 0;
	}
	...
	if (h->boundary_located)
	{
		...
		if (*fbc == '"') { BS_push(mo->ho->bo, (fbc +1)); MIMEH_set_defect( hinfo, MIMEH_DEFECT_UNBALANCED_BOUNDARY_QUOTE); }
		if (*lbc == '"') { *lbc = '\0'; BS_push(mo->ho->bo, h->boundary); *lbc = '"'; MIMEH_set_defect( hinfo, MIMEH_DEFECT_UNBALANCED_BOUNDARY_QUOTE); }
		...
		if (h->content_type == _CTYPE_RFC822)
		{
			...
			result = MIME_handle_rfc822(f,mo,h,current_recursion_level,ss);
			...
			result = MIME_handle_multipart(f, mo, h, current_recursion_level, ss);

		} else {

			if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_unpack_stage2:DEBUG: Decoding boundaryless file (%s)...\n",FL,h->filename);

			result = MIME_handle_plain( f, mo,h,current_recursion_level,ss);
	...
	if ((BS_top(mo->ho->bo)!=NULL)&&(result == 0))
	{
		...
		if (result == 0 || result == MIME_STATUS_ZERO_FILE)
		{

			if (BS_top(mo->ho->bo)!=NULL)
			{
					...
					if (result == 0)
					{
							...
							)
						{
							...
							else 
							{
								...
								mime_fname = PLD_dprintf("%s/%s", mo->unpackdir, h->filename);
								if(mime_fname != NULL)
								{
									...
									if (MIME_is_file_RFC822(mime_fname))
									{
										...
										MIME_unpack_single(mo, mime_fname, current_recursion_level+1,ss);
									}
									free(mime_fname);
								}
							}
...
int MIME_unpack_mailbox(struct MIME_object *mo, char *mpname, int current_recursion_level, struct SS_object *ss )
{
	...
	snprintf(fname,sizeof(fname),"%s/tmp.email000.mailpack",mo->unpackdir);
	...
	while (FFGET_fgets(line,sizeof(line),&f))
	{
		...
		if ((lastlinewasblank==1)&&(strncasecmp(line,"From ",5)==0))
		{
			...
			// 20040317-2358:PLD
			MIME_unpack_single(mo, fname, current_recursion_level ,ss );
			...
			snprintf(fname,sizeof(fname),"%s/tmp.email%03d.mailpack",mo->unpackdir,++mcount);
			fo = fopen(fname,"w");
		}
		else
		{
...
int MIME_unpack_single(struct MIME_object *mo, char *mpname, int current_recursion_level, struct SS_object *ss )
{
	...
	// 20040317-2359:PLD
	result = MIME_unpack_single_fp(mo,fi,current_recursion_level , ss);
...
Input:         struct MIME_object *mo : information of current unpacking file
...
int MIME_unpack_single_fp(struct MIME_object *mo, FILE *fi, int current_recursion_level, struct SS_object *ss )
{
	...
	if ((!hf)&&(glb.save_headers)&&(MIMEH_get_headers_save()==0))
	{
		...
		snprintf(h.scratch,sizeof(h.scratch),"%s/%s",mo->unpackdir, glb.headersname);
		hf = fopen(h.scratch,"w");
		if (!hf)
		{
			glb.save_headers = 0;
		...
		else
		{
			...
			MIMEH_set_headers_save(mo->ho, hf);
		}
	}
	...
	if ( headers_save_set_here > 0 )
	{
		...
		fflush(stdout);
		MIMEH_set_headers_nosave(mo->ho);
		fclose(hf);
	}
...
Errors:
------------------------------------------------------------------------*/
int MIME_unpack(struct MIME_object *mo, char *mpname, int current_recursion_level, struct email_attach_file **output)
{
	int result = 0;
	...
	if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_unpack: Unpacking %s to %s, recursion level is %d",FL,mpname,mo->unpackdir,current_recursion_level);

	MIMEH_set_outputdir(mo->ho, mo->unpackdir);
	...
	if (glb.mailbox_format > 0)
	{
		...
		result = MIME_unpack_mailbox(mo, mpname, (current_recursion_level), &ss );
	}
	else
	{
		if (MIME_DNORMAL) LOGGER_log("%s:%d:MIME_unpack: Unpacking standard mailpack",FL,mpname,mo->unpackdir,current_recursion_level);
		result = MIME_unpack_single(mo, mpname, (current_recursion_level +1), &ss );
	}

	if (glb.no_nameless)
	{
		MIME_postdecode_cleanup(mo->unpackdir, &ss );
	}
...
static int _copy_ss(struct SS_object *ss, struct email_attach_file **output)
{
	...
	struct email_attach_file *head;
	while (n != NULL) {
		char *sharp = strchr(n->data, '#');
		if (!sharp) {
			//Wrong format of output long string
			continue;
		}
		head = *output;
		*output = (struct email_attach_file *)malloc(sizeof(struct email_attach_file));
		(*output)->name = strndup(n->data, sharp - n->data);
		(*output)->path = strdup(sharp + 1);
		(*output)->nxt = head;
		n = n->next;
	}
	return 0;
...
int MIME_close(struct MIME_object *mo)
{
	free(mo->ho);
	free(mo);
	return 0;
